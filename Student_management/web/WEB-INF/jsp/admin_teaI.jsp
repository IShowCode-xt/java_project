            ...
            <div class="result-content">
                <table class="result-tab" width="100%" id ="vetListTable">
                    ...
                    <c:forEach var="teacher" items="${list}">
                        <tr>
                            <td>
                                <input name="id[]" value="${teacher.id}" type="checkbox">
                            </td>
                            <td>${teacher.id}</td>
                            <td>${teacher.username}</td>
                            <td>${teacher.classId}</td>
                            <td>
                                <a class="link-update" href='AdminfindTeaById.do?id=${teacher.id}'>修改</a>
                                <a class="link-del" href='admindelectTea.do?id=${teacher.id}'>删除</a>
                            </td>
                        </tr>
                    </c:forEach>
                    </tbody>
...
<script >
    ...
    function st() {
        ...
        location.href="afindTea.do?type="+type+"&userSearchTxt="+userSearchTxt;
    }
    function goPage1() {
        var userSearchTxt=$("#userSearchTxt").val();
        var type=$("#queryTypeSelect").val();
        var pagenumber=1;
        location.href="afindTeaPage.do?type="+type+"&pageNumber="+pagenumber+"&userSearchTxt="+userSearchTxt;
    }
    function goPage2() {
        var userSearchTxt=$("#userSearchTxt").val();
        var type=$("#queryTypeSelect").val();
        ...
        location.href="afindTeaPage.do?type="+type+"&pageNumber="+pagenum+"&userSearchTxt="+userSearchTxt;
    }
    function goPage3() {
        var userSearchTxt=$("#userSearchTxt").val();
        var type=$("#queryTypeSelect").val();
        ...
        var pagenum=pageNum+1;
        location.href="afindTeaPage.do?type="+type+"&userSearchTxt="+userSearchTxt+"&pageNumber="+pagenum;
    }
    function goPage4() {
        var userSearchTxt=$("#userSearchTxt").val();
        var type=$("#queryTypeSelect").val();
